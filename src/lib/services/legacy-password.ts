...
/**
 * Whether this user must change their password before using the app: only when the policy is on
 * *and* the hash is still bcrypt. Changing it rehashes with argon2id, so the gate self-resolves.
 * Federated users have no password hash and are never caught by it.
 */
