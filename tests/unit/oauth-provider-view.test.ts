...
describe('OAuth provider browser boundary', () => {
  it("uses Better Auth 1.7's standard, path-safe social callback URL", () => {
    expect(oauthCallbackUrl('https://cpm.example.com/', 'team/provider')).toBe(
      'https://cpm.example.com/api/auth/callback/team%2Fprovider',
    );
    expect(oauthCallbackUrl('https://cpm.example.com', 'provider-id')).not.toContain(
      '/oauth2/callback/',
    );
  });
