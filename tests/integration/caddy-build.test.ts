...
describe('applyCaddyBuild', () => {
  ...
  it('does not signal a rebuild when the config apply fails', async () => {
    // Writing the trigger anyway would hand the sidecar a rebuild whose new
  ...
  it('writes an override and a trigger the sidecar can act on', async () => {
...
describe('build status', () => {
  it('is idle before the sidecar has written anything', () => {
    expect(getCaddyBuildStatus()).toEqual({ state: 'idle' });
  });

  it('reads what the sidecar wrote', () => {
    writeFileSync(
      STATUS_PATH,
  ...
  it('falls back to idle rather than throwing on a truncated status file', () => {
    // The sidecar writes this file while the web container may be reading it.
