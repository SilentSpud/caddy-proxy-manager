#!/bin/sh
#
# Caddy Compose Manager Agent (historically the "L4 port manager")
#
...
# and clears any stale "building" status from a agent killed mid-build.
#
...
build_compose_args() {
  ...
  # than the agent's /compose mount.
  ...
  # when --project-directory points to a host path not mounted in the agent.
...
do_apply() {
  ...
  # "Waiting for port manager agent..." on the next boot.
  rm -f "$TRIGGER_FILE"
...
if [ -f "$OVERRIDE_FILE" ]; then
  ...
  write_status "idle" "Port manager agent is running and ready."
...
if [ -f "$BUILD_STATUS_FILE" ] && grep -q '"state": *"\(building\|pending\)"' "$BUILD_STATUS_FILE" 2>/dev/null; then
  STALE_MSG="The agent restarted while a rebuild was in progress. The Caddy image was left unchanged; click Rebuild to try again."
