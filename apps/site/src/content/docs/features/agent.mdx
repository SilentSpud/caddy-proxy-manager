...
**Settings → Agents** - six letters, valid five minutes, single use, at most five wrong guesses a
...
travels over this link. The two exchange a secret, stored encrypted on both sides, and the code is
never used again. An agent that is already paired re-pairs only with a code from **Re-pair** on its
row. `CONTROLLER_URL` and `PAIRING_CODE` do the same without a terminal.
