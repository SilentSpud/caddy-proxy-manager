...
certificate, access list, agent assignment, redirects, rewrites and location rules, path rules,
error pages, raw Caddy config, external forward auth, Authentik, Tailscale, load balancing, DNS
resolution, geo-blocking, WAF, mTLS trust and the HSTS and hostname-validation toggles. Pinning it to agents works the way it does for a stored host - nothing ticked means every
agent.
