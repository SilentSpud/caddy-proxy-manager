...
**An empty list fails closed.** A host whose list has no members refuses every request rather than
serving it unauthenticated. The access lists page flags a list with no members, and warns when a
host is still using one; the host editor marks empty lists in its picker and warns when one is
chosen.
