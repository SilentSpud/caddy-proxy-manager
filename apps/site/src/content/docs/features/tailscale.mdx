...
**Reach upstreams over the tailnet** reaches a service that only exists on the tailnet, without exposing
it anywhere else.
...
Pairing, the event stream and long-lived connections behave as they do on a flat network.
