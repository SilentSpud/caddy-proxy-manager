...
That is the whole file. Those two are the only variables Compose has no default for — it declares
them `${SESSION_SECRET:?}` and `${POSTGRES_PASSWORD:?}` and refuses to start rather than come up on
something guessable — so they are generated rather than chosen, and nothing you would have had to
think of ends up in the file.
