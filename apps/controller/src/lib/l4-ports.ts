 ...
 * web writes docker-compose.l4-ports.yml plus l4-ports.trigger, the agent runs
...
import { AGENT_FILES, type L4PortsStatus } from "@cpm/shared";
import db from "./db";
import { l4ProxyHosts } from "./db/schema";
import { eq } from "drizzle-orm";

const DATA_DIR = process.env.L4_PORTS_DIR || "/app/data";
const { override: OVERRIDE_FILE, trigger: TRIGGER_FILE, status: STATUS_FILE } = AGENT_FILES.l4Ports;
...
`;
  ...
  return {
    state: "pending",
    message: `Trigger written. Waiting for the agent to apply ${requiredPorts.length} port(s).`,
    triggeredAt,
  };
}

/** Read the current status from the status file written by the agent. */
...
/**
 * Whether the agent is available, judged by the status file or a processed trigger file.
 */
