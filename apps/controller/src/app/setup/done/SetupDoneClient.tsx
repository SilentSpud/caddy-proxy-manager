        ...
        <FormCard title={t("environmentCleanupTitle")}>
          ...
          {cleanup.keep.length > 0 && (
            <Banner
              ...
              description={`Docker Compose provisions the clickhouse container from ${cleanup.keep.join(", ")}, and it cannot read the database - so the command above leaves them alone. Without an agent they have to stay: Docker is the only thing that can start that container there. With an agent the saved values are passed to Compose for you, and these lines can go as well - but drop clickhouse from COMPOSE_PROFILES at the same time, or your own docker compose up -d keeps recreating the container from the now-stale values in the file.`}
            />
          )}
        </FormCard>
