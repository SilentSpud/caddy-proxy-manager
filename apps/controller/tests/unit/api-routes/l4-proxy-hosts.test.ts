...
import { GET as listGET, POST } from '@/src/app/api/v1/l4-proxy-hosts/route';
import { GET as getGET, PUT, DELETE } from '@/src/app/api/v1/l4-proxy-hosts/[id]/route';
import {
  listL4ProxyHosts,
  createL4ProxyHost,
  getL4ProxyHost,
...
describe('PUT /api/v1/l4-proxy-hosts/[id]', () => {
  ...
  it('returns 500 when host not found', async () => {
    ...
    const response = await PUT(createMockRequest({ method: 'PUT', body: { listen_port: 4444 } }), {
