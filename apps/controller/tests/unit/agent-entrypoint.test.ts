/**
 * Invariants of the compose-manager agent's entrypoint script. It runs in its own container, so
...
const SCRIPT_PATH = resolve(moduleDir, '../../../agent/entrypoint.sh');
...
describe('agent entrypoint.sh', () => {
  ...
  it('bounds the build with a timeout so a hung compile cannot wedge the agent', () => {
  ...
  it('clears a stale in-progress build status on startup', () => {
    // A agent killed mid-build leaves state=building on disk, and the trigger is pre-loaded as
  ...
  it('uses --project-directory $COMPOSE_HOST_DIR when COMPOSE_HOST_DIR is set', () => {
    ...
    // directory, not the agent's /compose mount — --project-directory tells the daemon where.
  ...
  it('always reads compose files from $COMPOSE_DIR regardless of COMPOSE_HOST_DIR', () => {
    // The agent mounts the project at /compose (COMPOSE_DIR). Set or not, all -f flags must
