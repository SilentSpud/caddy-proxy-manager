...
test.describe
  .serial('mTLS HTTPS enforcement', () => {
    test.setTimeout(180_000);

    test('setup: import server certs, generate CAs, issue clients, and create mTLS hosts', async ({
      page,
    }) => {
    ...
    test('enforces mTLS only on configured protected paths', async () => {
      ...
      const protectedWithTrustedCert = await httpsGet(
        PROTECTED_PATHS_DOMAIN,
        '/admin/panel',
        tlsIdentity(clientA),
      );
      ...
      const nestedProtectedWithTrustedCert = await httpsGet(
        PROTECTED_PATHS_DOMAIN,
        '/internal/status',
        tlsIdentity(clientA),
      );
      ...
      expectMtlsBlocked(
        await httpsGetOutcome(PROTECTED_PATHS_DOMAIN, '/admin/panel', tlsIdentity(clientB)),
      );
    });
    ...
    test('bypasses mTLS on excluded paths while keeping other paths protected', async () => {
      ...
      expectMtlsBlocked(
        await httpsGetOutcome(EXCLUDED_PATHS_DOMAIN, '/private', tlsIdentity(clientB)),
      );
    });

    test('revokes a tracked client certificate and blocks it while leaving other active certs usable', async ({
      page,
    }) => {
    ...
    test('fails closed when the only issued client certificate for a CA is revoked', async ({
      page,
    }) => {
