...
async function insertEntry(
  accessListId: number,
  overrides: Partial<typeof accessListEntries.$inferInsert> = {},
) {
  const now = nowIso();
...
describe('access-lists integration', () => {
  ...
  it('deletes a list and cascades to entries', async () => {
    ...
    const listRow = await db.query.accessLists.findFirst({
      where: (t, { eq }) => eq(t.id, list.id),
    });
  ...
  it('entries for different lists do not mix', async () => {
    ...
    const list1Entries = await db
      .select()
      .from(accessListEntries)
      .where(eq(accessListEntries.accessListId, list1.id));
